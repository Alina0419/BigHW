...
***************************************************************************/
bool is_tab_character(const char* p, const char tabs[][4], int tabs_size)
{
	for (int i = 0; i < tabs_size; ++i)
		if (strncmp(p, tabs[i], 2) == 0)
			return true;
	return false;
}
void cct_showstr(const int X, const int Y, const char* str, const int bg_color, const int fg_color, int rpt, int maxlen)
{
	...
	for (i = 0, p = str; i < maxlen; i++, p++) {
		...
		if (rpt_count < rpt)
		{
			if (is_tab_character(p, tabs, tabs_size))
			{
				// 输出2字节的特殊字符并添加空格
				putchar(*p++);
				i++;
